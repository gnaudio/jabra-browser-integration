...
const noteHeadsetSelectionProblem = document.getElementById(
  "noteHeadsetSelectionProblem"
);
...
const noiseChartContext = document
  .getElementById("noiseChart")
  .getContext("2d");
const exposureChartContext = document
  .getElementById("exposureChart")
  .getContext("2d");

const overviewChartContext = document
...
const collapseHeadSetSelected = document.getElementById(
  "collapseHeadSetSelected"
);
...
const collapseBackgroundNoise = document.getElementById(
...
function run(cppAccountUrl, quickPhoneNumber, elasticsearchHost) {
  ...
  function deviceHasLogging(device) {
    return (
      ...
      device.deviceFeatures.includes(jabra.DeviceFeature.Logging)
    );
  }
  ...
  function deviceHasMMIv2(device) {
    return (
      device &&
      device.deviceFeatures &&
      device.deviceFeatures.includes(jabra.DeviceFeature.RemoteMMIv2)
    );
  }
  ...
  function weightedTimeAvg(
    ...
    if (dbTimestamps.length > 0) {
      ...
      let dbTimedList = dbTimestamps
        .filter(
          currentValue => !sinceTimeStamp || currentValue.ts > sinceTimeStamp
        )
        .map((currentValue, index, array) => {
          let nextTimeStamp =
            index + 1 < array.length ? array[index + 1].ts : endTimeStamp;
      ...
      let dividendAndDivisor = dbTimedList.reduce(
        (acc, currValue) => {
          return {
      ...
      // Return result
      return dividendAndDivisor.time !== 0.0
        ? dividendAndDivisor.sum / dividendAndDivisor.time
        : 0;
    } else {
      return undefined;
    }
  ...
  setupExpandCollapse(
    headSetSelectedContent,
    expandHeadSetSelected,
    collapseHeadSetSelected
  );
  setupExpandCollapse(boomArmContent, expandBoomArm, collapseBoomArm);
  setupExpandCollapse(
    backgroundNoiseContent,
    expandBackgroundNoise,
    collapseBackgroundNoise
  );
  setupExpandCollapse(
    exposureContent,
    expandExposureNoise,
    collapseExposureNoise
  );
  setupExpandCollapse(
    callOverviewContent,
    expandCallOverview,
    collapseCallOverview
  );
  ...
  function reportLiveToElasticSearchCloud(live) {
    if (es_client && activeContact) {
      // Report only if enabled and contact is active
      ...
      if (activeAgent) {
        callInfo = {
          ...
          contactPhoneNumber: activeContact.getInitialConnection().getEndpoint()
            .phoneNumber
        };
      ...
      if (
        ...
        if (live) {
          const txSpeechIntervalTime =
            txSpeechReportTime - lastTxSpeechReportTime;
          const rxSpeechIntervalTime =
            rxSpeechReportTime - lastRxSpeechReportTime;
          const crossTalkIntervalTime =
            crossTalkReportTime - lastCrossTalkReportTime;
          const silenceIntervalTime = silenceReportTime - lastSilenceReportTime;

          const intervalTotal =
            txSpeechIntervalTime +
            rxSpeechIntervalTime +
            crossTalkIntervalTime +
            silenceIntervalTime;
          ...
          analytics["txSpeechNowPct"] =
            (100.0 * txSpeechIntervalTime) / intervalTotal;
          analytics["rxSpeechNowPct"] =
            (100.0 * rxSpeechIntervalTime) / intervalTotal;
          analytics["crossTalkNowPct"] =
            (100.0 * crossTalkIntervalTime) / intervalTotal;
          analytics["silenceNowPct"] =
            (100.0 * silenceIntervalTime) / intervalTotal;
        }
      }
      ...
      let status = undefined;
      {
        ...
        if (loggingDevice) {
          ...
          status["audioExposureAvg"] = weightedTimeAvg(
            audioExposureQueue,
            timeThisReport.getTime()
          );
          status["backgroundNoiseAvg"] = weightedTimeAvg(
            backgroundNoiseQueue,
            timeThisReport.getTime()
          );

          if (live) {
            status["audioExposureNow"] = weightedTimeAvg(
              audioExposureQueue,
              timeThisReport.getTime(),
              lastReportLiveToElasticSearchCloud
                ? lastReportLiveToElasticSearchCloud.getTime()
                : undefined
            );
            status["backgroundNoiseNow"] = weightedTimeAvg(
              backgroundNoiseQueue,
              timeThisReport.getTime(),
              lastReportLiveToElasticSearchCloud
                ? lastReportLiveToElasticSearchCloud.getTime()
  ...
  jabra
    ...
    .then(_installInfo => {
      ...
      if (_installInfo.installationOk) {
        ...
        return Promise.reject(
          new Error("Browseer SDK Installation incomplete. Please (re)install")
        );
      }
    })
    .then(() => {
    ...
    .catch(err => {
      if (
        ...
        err.errmessage === "Unknown cmd" &&
        err.command === "getinstallinfo"
      ) {
        showError(
          "Your browser SDK installation is incomplete, out of date or corrupted. Please (re)install"
        );
    ...
    return jabra
      ...
      .then(device => {
        ...
        if (device && device.deviceName) {
          addOnHeaderText.innerText = device.deviceName
            ? device.deviceName
            : "";
          if (deviceHasMMIv2(activeDevice)) {
            jabra
              .setMmiFocus(jabra.RemoteMmiType.MMI_TYPE_DOT3, true)
              .then(() => {
                return jabra.setMmiFocus(
  ...
  jabra.addEventListener("mmi", event => {
    if (
      event.data.type === jabra.RemoteMmiType.MMI_TYPE_DOT3 &&
      event.data.action === jabra.RemoteMmiActionInput.MMI_ACTION_UP
    ) {
      const availableState = activeAgent
        .getAgentStates()
      ...
      event.data.type === jabra.RemoteMmiType.MMI_TYPE_DOT4 &&
      event.data.action === jabra.RemoteMmiActionInput.MMI_ACTION_UP &&
      quickPhoneNumber
    ) {
      ...
      if (state === connect.AgentStateType.ROUTABLE) {
        ...
        activeAgent.connect(endpoint, {
          ...
          failure: jsonErr => {
            console.error(
              "Failed calling supervisor: " + JSON.stringify(jsonErr)
            );
          }
        });
      } else {
        console.error(
          "Can not call supervisor unless routable (state = " + state + ")"
        );
      }
    }
  });
  ...
  jabra.addEventListener("devlog", event => {
    ...
    currentTxSpeechTime = txSpeechStart
      ? Math.abs((timeStamp - txSpeechStart) / 1000)
      : 0;
    ...
    currentRxSpeechTime = rxSpeechStart
      ? Math.abs((timeStamp - rxSpeechStart) / 1000)
      : 0;
    ...
    currentCrossTalkTime = crossTalkStart
      ? Math.abs((timeStamp - crossTalkStart) / 1000)
      : 0;
  ...
  function calculateSilence(timeStamp) {
    if (
      lastTxSpeechOrStart &&
      lastRxSpeechOrStart &&
      (!txSpeechStart && !rxSpeechStart)
    ) {
      silence =
        Math.abs(timeStamp - lastTxSpeechOrStart) >= silenceMinDurationMs &&
        Math.abs(timeStamp - lastRxSpeechOrStart) >= silenceMinDurationMs;
    ...
    currentSilenceTime = silenceStart
      ? Math.abs((timeStamp - silenceStart) / 1000)
      : 0;
  ...
  function updateBoomArmGui(boomArm) {
    boomArmStatusText.innerText = boomArm
  ...
  function updateExposureGui(exposureDb) {
    ...
    if (
      !lastExposureDate ||
      date.getTime() - lastExposureDate.getTime() >= 5000
    ) {
    ...
    datasets: [
      {
        ...
        borderColor: "grey",
        borderWidth: 1,
        backgroundColor: ["#eb5757", "#ffd100", "#27AE60", "#808080"]
      }
    ]
  };
  ...
  function updateCallOverviewGui() {
    ...
    if (txSpeech !== undefined) {
      ...
      agentTalkingLabel.style.fontFamily = txSpeech
        ? "GNElliotWeb-Bold"
        : "GNElliotWeb-Regular";
    }
    ...
    if (rxSpeech !== undefined) {
      ...
      contactLabel.style.fontFamily = rxSpeech
        ? "GNElliotWeb-Bold"
        : "GNElliotWeb-Regular";
    }
    ...
    if (crossTalk !== undefined) {
      ...
      crossTalkLabel.style.fontFamily = crossTalk
        ? "GNElliotWeb-Bold"
        : "GNElliotWeb-Regular";
    }
    ...
    if (silence !== undefined) {
      ...
      silenceLabel.style.fontFamily = silence
        ? "GNElliotWeb-Bold"
        : "GNElliotWeb-Regular";
    }
  ...
  connect.contact(function(contact) {
    ...
    contact.onConnected(function(contact) {
      ...
      let cloudReportInterval = setInterval(
        activeContactOnSetup => {
          // Auto unsubscribe once call is finished and final report was made.
          if (
            activeContactOnSetup !== activeContact ||
            (!inCall && cloudReportInterval)
          ) {
...
let quickPhoneNumber = quickPhoneNumberParam
  ? decodeURI(quickPhoneNumberParam)
  : "";
let elasticsearchHost = elasticsearchHostParam
  ? decodeURI(elasticsearchHostParam)
  : "";
